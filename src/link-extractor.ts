import type { TranscriptInputItem } from './types.js';

const urlPattern = /https?:\/\/[^\s<>"']+/g;

export function extractEntries(text: string): TranscriptInputItem[] {
  const items: TranscriptInputItem[] = [];
  const lines = String(text || '').split(/\r?\n/);

  for (const line of lines) {
    const matches = [...line.matchAll(urlPattern)];
    if (matches.length === 0) continue;

    for (const match of matches) {
      let candidate = match[0].trim();
      while (candidate && '.,;:)]}'.includes(candidate.at(-1) ?? '')) {
        candidate = candidate.slice(0, -1);
      }

      let parsed: URL;
      try {
        parsed = new URL(candidate);
      } catch {
        continue;
      }

      const prefix = line.slice(0, match.index ?? 0).trim();
      const title = cleanTitle(prefix, items.length + 1);
      items.push({
        order: items.length,
        title,
        url: parsed.toString(),
      });
    }
  }

  return items;
}

export function extractLinks(text: string): string[] {
  return extractEntries(text).map((entry) => entry.url);
}

function cleanTitle(prefix: string, fallbackIndex: number): string {
  let title = String(prefix || '').trim();
  title = title.replace(/^\s*\d+\s*[\.\)]\s*/, '');
  title = title.replace(/[\-–—:|]+\s*$/, '').trim();
  return title || `Link ${fallbackIndex}`;
}
